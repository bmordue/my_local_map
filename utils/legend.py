...
class MapLegend:
    ...
    def _load_tourist_legend_items(self):
        ...
        self.items.extend(
            [
                LegendItem(
                    "Forest / Woodland", "polygon", fill="#d4e6b7", fill_opacity=0.8
                ),
                LegendItem(
                    "Farmland / Grassland", "polygon", fill="#e8f5d4", fill_opacity=0.6
                ),
                LegendItem(
                    "Parks / Gardens", "polygon", fill="#c8facc", fill_opacity=0.8
                ),
            ]
        )
        ...
        self.items.extend(
            [
                LegendItem("Water Bodies", "polygon", fill="#7dd3c0", fill_opacity=0.8),
                LegendItem(
                    "Rivers",
                    "line",
                    stroke="#7dd3c0",
        ...
        self.items.extend(
            [
                ...
                LegendItem(
                    "Religious Sites",
                    "icon",
                    icon_path="icons/religious-christian-15.svg",
                ),
            ]
        )
            ...
            "categories": {
                "Land Use": len(
                    [
                        i
                        for i in self.items
                        if "Forest" in i.label
                        or "Farmland" in i.label
                        or "Parks" in i.label
                    ]
                ),
                "Water Features": len(
                ...
                "Buildings & POIs": len(
                    [
                        i
                        for i in self.items
                        if "Building" in i.label or "Points of Interest" in i.label
                    ]
                ),
            },
        }
...
def add_legend_to_image(image_path, legend_data, output_path=None):
    ...
    with Image.open(image_path) as img:
        ...
        draw.rectangle(
            background_coords, fill=(255, 255, 255, 230), outline=(51, 51, 51), width=2
        )

        # Try to load font, fall back to default if not available
        try:
            title_font = ImageFont.truetype(
                "/usr/share/fonts/truetype/dejavu/DejaVu-Sans-Bold.ttf", 14
            )
            text_font = ImageFont.truetype(
                "/usr/share/fonts/truetype/dejavu/DejaVu-Sans.ttf", 11
            )
            ...
            if item.symbol_type == "polygon":
                ...
                if "stroke_dasharray" in item.properties:
                    ...
                    for i in range(0, symbol_size, 4):
                        if i + 2 <= symbol_size:
                            ...
                            draw.line(
                                line_coords, fill=stroke_color, width=stroke_width
                            )
                else:
                    # Solid line
                    line_coords = [
                ...
                if icon_path and Path(icon_path).exists():
                    try:
                        ...
                        logger.info(
                            f"Warning: Failed to render icon '{icon_path}': {e}"
                        )
                    ...
                    draw.text(
                        (symbol_x, symbol_y), "?", fill=(44, 62, 80), font=text_font
                    )

...
class OSDataProcessor:
    ...
    def download_os_data(
        self, product_key: str, bbox: Dict[str, float], output_file: str
    ) -> bool:
        """
    ...
    def _create_mock_os_data(
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                f"# In a real implementation, this would be downloaded from OS API\n"
            )
        ...
        return [
            {
                ...
                "class": "A Road",
                "name": "A944",
                "start_lat": str(bbox["south"]),
                "start_lon": str(center_lon),
                "end_lat": str(bbox["north"]),
                "end_lon": str(center_lon),
            },
            {
                "id": "2",
                "class": "B Road",
                "name": "B9119",
                "start_lat": str(center_lat),
                "start_lon": str(bbox["west"]),
                "end_lat": str(center_lat),
                "end_lon": str(bbox["east"]),
            },
            {
                "id": "3",
                "class": "Unclassified",
                "name": "Local Road",
                "start_lat": str(center_lat + 0.005),
                "start_lon": str(center_lon - 0.01),
                "end_lat": str(center_lat - 0.005),
                "end_lon": str(center_lon + 0.01),
            },
        ]

    def _create_mock_boundaries_csv(
        ...
        return [
            ...
            },
            {
                ...
                "type": "Parish",
                "name": "Lumsden Parish",
                "start_lat": str(bbox["south"] + 0.01),
                "start_lon": str(bbox["west"] + 0.01),
                "end_lat": str(bbox["north"] - 0.01),
                "end_lon": str(bbox["east"] - 0.01),
            },
        ]

    def _create_mock_rights_of_way_csv(
        self, bbox: Dict[str, float]
    ) -> List[Dict[str, str]]:
        ...
        return [
            {
                ...
                "name": "Lumsden Loop",
                "start_lat": str(center_lat),
                "start_lon": str(center_lon - 0.005),
                "end_lat": str(center_lat + 0.01),
                "end_lon": str(center_lon + 0.005),
            },
            {
                "id": "2",
                "type": "Bridleway",
                ...
                "start_lat": str(center_lat + 0.005),
                "start_lon": str(center_lon),
                "end_lat": str(center_lat + 0.015),
                "end_lon": str(center_lon + 0.01),
            },
        ]

    def convert_os_to_shapefiles(
        self, os_data_file: str, product_key: str
    ) -> Optional[str]:
        """
        ...
        if Path(os_data_file).exists():
            ...
            if csv_file.exists():
                ...
                print(
                    f"  ✓ Mock {product['name']} ready for integration (no conversion needed)"
                )
        ...
        try:
            if product_key == "terrain" and os_data_file.endswith(".asc"):
                ...
                cmd = ["gdal_translate", "-of", "GTiff", os_data_file, str(output_file)]
            else:
    ...
    def _create_mock_shapefile(
        self, csv_file: Path, output_dir: Path, product_key: str
    ) -> str:
        """
        ...
        print(
            f"  ⚠️ OS {product_key} layer will be disabled due to shapefile creation issues"
        )
        return None

    def process_os_data_for_area(
        self, bbox: Dict[str, float], enabled_layers: List[str] = None
    ) -> Dict[str, str]:
        """
        ...
        for layer_key in enabled_layers:
            ...
            if self.download_os_data(layer_key, bbox, str(download_file)):
                ...
                output_dir = self.convert_os_to_shapefiles(
                    str(download_file), layer_key
                )
                if output_dir:
                    ...
                    print(
                        f"  ⚠️ {product['name']} processing failed - layer will be disabled"
                    )
            else:
...
def integrate_os_data_with_map(
    bbox: Dict[str, float], os_config: Dict = None
) -> Dict[str, str]:
    """

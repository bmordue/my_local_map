...
class CoordinateValidator:
    ...
    def validate_coordinates(self, data: List[Dict]) -> ValidationResult:
        ...
        if invalid_coords == 0:
            logger.info(
                f"✓ Coordinate validation passed: {valid_count}/{len(data)} valid coordinates"
            )
        else:
            logger.warning(
                f"⚠ Coordinate validation issues: {invalid_coords} invalid, {out_of_bounds} out of bounds"
            )
    ...
    def _is_valid_coordinate(self, lat: float, lon: float) -> bool:
        ...
        return (
            self.valid_lat_range[0] <= lat <= self.valid_lat_range[1]
            and self.valid_lon_range[0] <= lon <= self.valid_lon_range[1]
        )
...
class AttributeValidator:
    ...
    def validate_attributes(
        self, data: List[Dict], data_type: str = "general"
    ) -> ValidationResult:
        """
        ...
        if incomplete_items == 0:
            logger.info(
                f"✓ Attribute validation passed: {complete_items}/{len(data)} items complete"
            )
        else:
            logger.warning(
                f"⚠ Attribute validation issues: {incomplete_items}/{len(data)} items incomplete"
            )
...
class TemporalValidator:
    ...
    def validate_temporal_data(self, data: List[Dict]) -> ValidationResult:
        ...
        if invalid_temporal == 0:
            logger.info(
                f"✓ Temporal validation passed: {valid_temporal} items with valid temporal data"
            )
        else:
            logger.warning(
                f"⚠ Temporal validation issues: {invalid_temporal} items with invalid temporal data"
            )
    ...
    def _validate_time_range(self, time_range: str) -> bool:
        ...
        if "-" in time_range:
            ...
            if len(parts) == 2:
                return (
                    self.time_pattern.match(parts[0].strip()) is not None
                    and self.time_pattern.match(parts[1].strip()) is not None
                )
...
class CrossReferenceValidator:
    ...
    def validate_cross_references(
        self, primary_data: List[Dict], reference_data: List[Dict]
    ) -> ValidationResult:
        """
        ...
        if conflicts_found == 0:
            logger.info(
                f"✓ Cross-reference validation passed: {matches_found} matches, no conflicts"
            )
        else:
            logger.warning(
                f"⚠ Cross-reference issues: {conflicts_found} conflicts found"
            )
    ...
    def _find_matches(
        self, primary_item: Dict, reference_data: List[Dict]
    ) -> List[Dict]:
        ...
        for ref_item in reference_data:
            ...
            if primary_name and ref_name:
                if (
                    ...
                    or self._name_similarity(primary_name, ref_name) > 0.8
                ):
            ...
            if primary_lat and primary_lon:
                ...
                if ref_lat and ref_lon:
                    distance = self._calculate_distance(
                        primary_lat, primary_lon, ref_lat, ref_lon
                    )
    ...
    def _check_conflicts(self, primary: Dict, reference: Dict) -> List[str]:
        ...
        for field in common_fields:
            if (
                ...
                and primary[field]
                and reference[field]
                and str(primary[field]).lower() != str(reference[field]).lower()
            ):
                conflicts.append(
                    f"'{field}' differs: '{primary[field]}' vs '{reference[field]}'"
                )
...
class QualityValidationReport:
    ...
    def generate_report(self, output_path: Optional[str] = None) -> Dict:
        ...
        report = {
            "validation_summary": {
                ...
                "overall_status": (
                    "PASSED" if all(r.passed for r in self.results) else "FAILED"
                ),
            },
    ...
    def _generate_recommendations(self) -> List[str]:
                ...
                if result.check_name == "coordinate_validation":
                    if result.stats.get("invalid_coordinates", 0) > 0:
                        recommendations.append(
                            "Review and correct invalid coordinate data"
                        )
                    ...
                    if completeness < 80:
                        recommendations.append(
                            "Improve data completeness - many items missing required fields"
                        )

                elif result.check_name == "temporal_validation":
                    if result.stats.get("invalid_temporal", 0) > 0:
                        recommendations.append(
                            "Standardize opening hours and seasonal information formats"
                        )
        ...
        if not recommendations:
            recommendations.append(
                "All quality checks passed - data quality is excellent!"
            )

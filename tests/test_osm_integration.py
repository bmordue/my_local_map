...
from utils.data_processing import validate_osm_data_quality, convert_osm_to_shapefiles, download_osm_data, calculate_bbox
import tempfile
import subprocess
...
class TestOSMIntegration:
    ...
    def test_shapefile_conversion_with_features(self):
        ...
        for shpname in shapefiles:
            ...
            try:
                result = subprocess.run(["ogrinfo", "-so", str(shp_file), shpname], 
    ...
    def test_bbox_calculation(self):
        ...
        assert 57.2 < bbox['south'] < bbox['north'] < 57.4, "Latitude should be in Lumsden range"
        assert -3.0 < bbox['west'] < bbox['east'] < -2.7, "Longitude should be in Lumsden range"

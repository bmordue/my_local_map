...
class TestTouristStyleXML:
    ...
    def tourist_xml_tree(self, tourist_xml_path):
        ...
        assert os.path.exists(tourist_xml_path), f"tourist.xml not found at {tourist_xml_path}"
    ...
    def test_tourist_xml_has_poi_style(self, tourist_xml_tree):
        ...
        root = tourist_xml_tree.getroot()
        poi_styles = [style for style in root.findall("Style") if style.get("name") == "poi"]
    ...
    def _could_use_regex_grouping(self, filter_text):
        ...
        if len(patterns) >= 2:
            ...
            for pattern in patterns[1:]:
                ...
                suffix = os.path.commonprefix([first_pattern[::-1], pattern[::-1]])[::-1]
    ...
    def test_specific_poi_categories_exist(self, tourist_xml_tree):
        ...
        expected_keywords = [
            "restaurant", "cafe", "pub",  # Food & Drink
            "hotel", "guest_house", "hostel", "chalet", "camp_site",  # Accommodation
            "bank", "post_office",  # Services
            "hospital", "pharmacy", "clinic",  # Healthcare
            "museum", "attraction", "viewpoint",  # Attractions
        ]
        ...
        assert len(missing_keywords) == 0, f"Missing expected POI keywords: {missing_keywords}"